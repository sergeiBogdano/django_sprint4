...
def get_page(request, queryset, paginate_by=PAGINATE_BY):
    ...
    return Paginator(queryset, paginate_by).get_page(page_number)
...
def index(request):
    ...
    processed_posts = process_posts(posts=post_queryset)
    page_obj = get_page(request, processed_posts)
...
def post_detail(request, post_id):
    post_queryset = process_posts(posts=Post.objects.filter(id=post_id),
                                  apply_filter=False)
...
def category_posts(request, category_slug):
    ...
    return render(request, 'blog/category.html', {
        ...
        'page_obj': get_page(
            request,
            process_posts(category.posts.all(), use_select_related=False)
        )
    })
...
def profile(request, username):
    ...
    page_obj = get_page(
        request,
        process_posts(author.posts.all(), apply_filter=False)
    )
